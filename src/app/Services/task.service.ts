...
export class TaskService {
  ...
  CreateTask(task: Task) {
    ...
    this.http.post<{ name: string }>('https://angularhttpclient-a330a-default-rtdb.firebaseio.com/tasks.json', task, { headers: headers })
      .subscribe({
        error: (err) => {
  ...
  GetAlltasks() {
    ...
    return this.http.get<{ [key: string]: Task }>(
      'https://angularhttpclient-a330a-default-rtdb.firebaseio.com/tasks.json'
      , { headers: headers, params: queryParams, observe:'response'}
    )
      .pipe(map((response) => {
        //Transform Data
        let tasks = [];
